...
export const AdminContests = ({ tournament }: { tournament: Tournament }) => {
  ...
  const sync = async (contest: Contest) => {
    ...
    try {
      ...
      setNotice(
        `Synchronized ${result.results.length} results; ${result.unmatchedHandles.length} unmatched handles.`,
      );
      refreshContests();
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Unable to synchronize results",
      );
    } finally {
      setBusy(false);
    }
                  ...
                  {contests.map((contest) => (
                    ...
                    >
                      ...
                      <td style={tdStyle}>
                        {contest.group
                          ? `Group ${contest.group}`
                          : `${contest.round} M${contest.matchNumber || ""}`}
                      </td>
                      <td style={tdStyle}>
...
export const AdminLogs = ({ tournamentId }: { tournamentId: string }) => {
  ...
  if (!logs.length)
    return <EmptyState label="No recorded admin actions yet." />;
