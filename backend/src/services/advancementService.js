...
const setParticipant = (id, status, currentRound) => Participant.findByIdAndUpdate(id, { status, currentRound }, { new: true });
const finishedContests = async (tournamentId, round, count) => {
  const contests = await Contest.find({ tournament: tournamentId, round });
  await Promise.all(contests.map(contestService.updateContestStatus));
  if (contests.length !== count || contests.some((contest) => contest.status !== "FINISHED")) throw new Error(`All ${count} ${round} contests must be finished`);
  return contests;
};
const resultForMatch = async (match) => {
  if (!match.contest) throw new Error(`Match ${match.matchNumber} has no contest`);
  const results = await ContestResult.find({ contest: match.contest, participant: { $in: match.participants } }).sort({ rank: 1 });
  if (results.length !== 2) throw new Error(`Match ${match.matchNumber} requires results for both participants`);
  return results;
};
const createMatches = (tournamentId, round, pairs) => Promise.all(pairs.map((participants, index) =>
...
const advanceGroupStage = async (tournamentId) => {
  ...
  for (const group of groups) {
    const members = await Participant.find({ tournament: tournamentId, group });
    const results = await ContestResult.find({ contest: byGroup.get(group)._id, participant: { $in: members.map((member) => member._id) } }).sort({ rank: 1 });
    if (members.length !== 5 || results.length < 2) throw new Error(`Group ${group} needs five participants and at least two results`);
    qualifiers[group] = results.slice(0, 2).map((result) => result.participant);
    await Promise.all(members.map((member) => setParticipant(member._id,
  ...
  const matches = await createMatches(tournamentId, "QUARTER_FINAL", [
    [qualifiers.A[0], qualifiers.B[1]], [qualifiers.C[0], qualifiers.D[1]],
    [qualifiers.B[0], qualifiers.A[1]], [qualifiers.D[0], qualifiers.C[1]],
  ]);
  tournament.status = "QUARTER_FINAL"; tournament.currentRound = "QUARTER_FINAL"; await tournament.save();
...
const advanceKnockout = async (tournamentId, round, nextRound, contestCount) => {
  ...
  for (const match of matches) {
    ...
    const winner = results[0].participant;
    match.winner = winner; await match.save(); winners.push(winner);
    await Promise.all(match.participants.map((participant) => setParticipant(participant,
      participant.equals(winner) ? "ADVANCED" : "ELIMINATED", participant.equals(winner) ? nextRound : "ELIMINATED")));
  }
  const pairs = round === "QUARTER_FINAL" ? [[winners[0], winners[1]], [winners[2], winners[3]]] : [[winners[0], winners[1]]];
  const nextMatches = await createMatches(tournamentId, nextRound, pairs);
  tournament.status = nextRound; tournament.currentRound = nextRound; await tournament.save();
...
const completeTournament = async (tournamentId) => {
  ...
  const results = await resultForMatch(match);
  const champion = results[0].participant;
  match.winner = champion; await match.save();
  await Promise.all(match.participants.map((participant) => setParticipant(participant,
    participant.equals(champion) ? "CHAMPION" : "ELIMINATED", participant.equals(champion) ? "CHAMPION" : "ELIMINATED")));
  tournament.status = "COMPLETED"; await tournament.save();
  return champion;
};

module.exports = { advanceGroupStage, advanceQuarterFinal: (id) => advanceKnockout(id, "QUARTER_FINAL", "SEMI_FINAL", 4), advanceSemiFinal: (id) => advanceKnockout(id, "SEMI_FINAL", "FINAL", 2), completeTournament };
